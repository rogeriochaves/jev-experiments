...
function loadKey() {
  const names = ["JEV_API_KEY", "TYPESAFE_API_KEY"];
  for (const n of names) if (process.env[n]) return process.env[n];
  const home = process.env.HOME ?? "";
  const files = [
    join(HERE, ".env"),
    join(HERE, "..", ".env"),
    join(home, "Projects/langwatch/platform/app/.env"),
  ];
  for (const f of files) {
    if (!existsSync(f)) continue;
    for (const line of readFileSync(f, "utf8").split("\n")) {
      const m = line.match(/^(JEV_API_KEY|TYPESAFE_API_KEY)=\s*"?([^"\s]+)"?/);
      if (m) return m[2];
    }
  }
  throw new Error("no key: set JEV_API_KEY or TYPESAFE_API_KEY, or put it in prose-lint/.env");
}
